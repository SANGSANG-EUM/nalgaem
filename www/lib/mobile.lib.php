...
function mobile_goods_review($name, $cnt, $gs_id, $rows=10)
{
	...
	echo "<div class=sp_vbox_mr>\n";
		echo "<ul>\n";
			...
			echo "<li class='trst'><a href=\"javascript:window.open('".BV_MSHOP_URL."/view_user.php?gs_id=$gs_id');\">더보기</a><span class='im im_arr'></span></li>\n";
		echo "</ul>\n";
	echo "</div>\n";
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if(is_admin() || ($member['id'] == $row['mb_id'])) {
			echo "&nbsp;&nbsp;&nbsp;<a href=\"javascript:window.open('".BV_MSHOP_URL."/orderreview.php?gs_id=$row[gs_id]&me_id=$row[index_no]&w=u');\" /><span class='under fc_blk'>수정</span></a>&nbsp;&nbsp;&nbsp;<a href=\"".BV_MSHOP_URL."/orderreview_update.php?gs_id=$row[gs_id]&me_id=$row[index_no]&w=d&hash=$hash\" class='itemqa_delete'><span class='under fc_blk'>삭제</span></a>";
		}
		echo "</span></span>";
		echo "</li>\n";
	}
...
function mobile_goods_qa($name, $cnt, $gs_id)
{
	...
	echo "<div class=sp_vbox_qa>\n";
		echo "<ul>\n";
			echo "<li class='tlst'>$name <span class=cate_dc>($cnt)</span></li>\n";
			echo "<li class='trst'><a href=\"javascript:window.open('".BV_MSHOP_URL."/qaform.php?gs_id=$gs_id');\" class='btn_lsmall bx-white'>Q&A쓰기</a></li>\n";
		echo "</ul>\n";
	echo "</div>\n";
	...
	for($i=0; $row=sql_fetch_array($result); $i++) {
		...
		$is_secret = false;
		if($row['iq_secret']) {
			$icon_secret = "<img src='".BV_IMG_URL."/icon/icon_secret.jpg' class='vam' alt='비밀글'>";
		...
		echo "<li class='lst'>\n$iq_subject";
			...
			echo "<div class='faq' id='qna".$i."' style='display:none;'>\n";
				...
				echo "<tbody>\n";
				echo "<tr>\n";
					echo "<td class='mi_dt'><img src='".BV_IMG_URL."/sub/FAQ_Q.gif'></td>\n";
					echo "<td class='mi_bt fc_125'>\n".nl2br($row['iq_question']);

					if(is_admin() || $member['id' ] == $row['mb_id'] && !$iq_answer) {
						echo "<div class='padt10'><a href=\"javascript:window.open('".BV_MSHOP_URL."/qaform.php?gs_id=$row[gs_id]&iq_id=$row[iq_id]&w=u');\" /><span class='under fc_blk'>수정</span></a>&nbsp;&nbsp;&nbsp;<a href=\"".BV_MSHOP_URL."/qaform_update.php?gs_id=$row[gs_id]&iq_id=$row[iq_id]&w=d&hash=$hash\" class='itemqa_delete'><span class='under fc_blk'>삭제</span></a></div>\n";
					}
					echo "</td>\n";
				echo "</tr>\n";
				...
				if($iq_answer) {
					echo "<tr>\n";
						echo "<td class='mi_dt padt20'><img src='".BV_IMG_URL."/sub/FAQ_A.gif'></td>\n";
