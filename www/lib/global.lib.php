...
function is_mobile()
{
	return preg_match('/'.BV_MOBILE_AGENT.'/i', $_SERVER['HTTP_USER_AGENT']);
}
...
function delete_board_thumbnail($boardid, $file)
{
    ...
    $files = glob(BV_DATA_PATH.'/board/'.$boardid.'/thumb-'.$fn.'*');
    if (is_array($files)) {
...
function check_string($str, $options)
{
    ...
    for($i=0;$i<strlen($str);$i++) {
        ...
        // 숫자
            ...
            if($options & BV_NUMERIC) {
                $s .= $c;
            }
        }
        // 영대문자
        else if($oc >= 0x41 && $oc <= 0x5A) {
            if(($options & BV_ALPHABETIC) || ($options & BV_ALPHAUPPER)) {
                $s .= $c;
            }
        }
        // 영소문자
        else if($oc >= 0x61 && $oc <= 0x7A) {
            if(($options & BV_ALPHABETIC) || ($options & BV_ALPHALOWER)) {
                $s .= $c;
            }
        }
        // 공백
...
{
    $f = file(BV_PLUGIN_PATH.'/htmlpurifier/safeiframe.txt');
    $domains = array();
    foreach($f as $domain){
        // 첫행이 # 이면 주석 처리
    ...
    include_once(BV_PLUGIN_PATH.'/htmlpurifier/HTMLPurifier.standalone.php');
    $config = HTMLPurifier_Config::createDefault();
    // data/cache 디렉토리에 CSS, HTML, URI 디렉토리 등을 만든다.
    $config->set('Cache.SerializerPath', BV_DATA_PATH.'/cache');
...
function rm_rf($file)
{
    if(file_exists($file)) {
        if(is_dir($file)) {
            ...
            @chmod($file, BV_DIR_PERMISSION);
            @rmdir($file);
        } else {
            @chmod($file, BV_FILE_PERMISSION);
            @unlink($file);
        }
    }
}
...
// 동일한 host url 인지
function check_url_host($url, $msg='', $return_url=BV_URL)
{
    if(!$msg)
...
{
    if(defined('BV_STRING_ENCRYPT_FUNCTION') && BV_STRING_ENCRYPT_FUNCTION) {
        $encrypt = call_user_func(BV_STRING_ENCRYPT_FUNCTION, $str);
    } else {
...
function get_sns_share_link($sns, $url, $title, $image_url)
{
	...
	$sns_send = BV_LIB_URL.'/sns_send.php?longurl='.urlencode($url).'&amp;title='.urlencode($title);
			...
			switch($cp['cp_type']){
					...
					if(($cp['cp_pub_sdate'] <= BV_TIME_YMD || $cp['cp_pub_sdate'] == '9999999999') &&
					   ($cp['cp_pub_edate'] >= BV_TIME_YMD || $cp['cp_pub_edate'] == '9999999999')) {
						...
						for($j=0; $j<count($cp_week_day); $j++) {
							for($k = 1; checkdate(BV_TIME_MONTH, $k, BV_TIME_YEAR); $k ++) {
								$thismonth = BV_TIME_MONTH.'/'.substr(sprintf('%02d',$k), -2);
						...
						$cnt = substr_count($is_week, BV_TIME_DAY);
						if($cnt) {
							//쿠폰발행 시간
							$tmp_cpdown = array();
							for($j=1; $j<=3; $j++) {
				...
				case '3': // 회원 생일자 발급
					...
					if($is_check_vars) {
						...
						if(BV_TIME_YMD >= $fr_birthday && BV_TIME_YMD <= $to_birthday) {
							$tmp_coupon[] = $cp['cp_id'];
						}
					}
					break;
...
// DB 연결
function sql_connect($host, $user, $pass, $db=BV_MYSQL_DB)
{
    if(function_exists('mysqli_connect') && BV_MYSQLI_USE) {
...
{
    if(function_exists('mysqli_select_db') && BV_MYSQLI_USE)
...
function sql_set_charset($charset, $link=null)
{
    ...
    if(function_exists('mysqli_set_charset') && BV_MYSQLI_USE)
...
function sql_query($sql, $error=BV_DISPLAY_SQL_ERROR, $link=null)
{
    global $tb;
    ...
    if(function_exists('mysqli_query') && BV_MYSQLI_USE) {
        if($error) {
...
function sql_fetch($sql, $error=BV_DISPLAY_SQL_ERROR, $link=null)
{
    global $tb;
...
{
    if(function_exists('mysqli_fetch_assoc') && BV_MYSQLI_USE)
...
{
    if(function_exists('mysqli_free_result') && BV_MYSQLI_USE)
...
function sql_insert_id($link=null)
{
    ...
    if(function_exists('mysqli_insert_id') && BV_MYSQLI_USE)
...
{
    if(function_exists('mysqli_num_rows') && BV_MYSQLI_USE)
...
{
    if(function_exists('mysqli_fetch_row') && BV_MYSQLI_USE)
...
function sql_field_names($table, $link=null)
{
    ...
    if(function_exists('mysqli_fetch_field') && BV_MYSQLI_USE) {
...
function sql_error_info($link=null)
{
    ...
    if(function_exists('mysqli_error') && BV_MYSQLI_USE) {
		...
		if($sm['cf_icode_id'] && $sm['cf_icode_pw']) {
			...
			if($userinfo['code'] == 0) {
				if($userinfo['payment'] == 'C') { // 정액제
					...
					$minimum_coin = 100;
					if(defined('BV_ICODE_COIN'))
						$minimum_coin = intval(BV_ICODE_COIN);
		...
		if($is_sms_send)
		{
			...
			if($sms_count > 0) {
				...
				if($sm['cf_sms_type'] == 'LMS') {
					include_once(BV_LIB_PATH.'/icode.lms.lib.php');
					...
					include_once(BV_LIB_PATH.'/icode.sms.lib.php');
...
function member_delete($mb_id)
{
	...
	while($row = sql_fetch_array($res)) {
		...
		$sql = "insert into shop_leave_log ( new_id, old_id, mb_id, reg_time, memo )
				values ( '$mb[pt_id]','$mb_id','$row[id]','".BV_TIME_YMDHIS."','$memo' )";
		sql_query($sql);
	}
...
{
	$skin_path = BV_PATH.'/m/theme/'.$skin;
...
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0)
{
    ...
    if($config['cf_point_term'] > 0) {
        if($expire > 0)
            $po_expire_date = date('Y-m-d', strtotime('+'.($expire - 1).' days', BV_SERVER_TIME));
        else
            $po_expire_date = date('Y-m-d', strtotime('+'.($config['cf_point_term'] - 1).' days', BV_SERVER_TIME));
    }

    $po_expired = 0;
    if($point < 0) {
        $po_expired = 1;
        $po_expire_date = BV_TIME_YMD;
    }
    $po_mb_point = $mb_point + $point;

    $sql = " insert into shop_point
                set mb_id = '$mb_id',
                    po_datetime = '".BV_TIME_YMDHIS."',
...
function delete_use_point($mb_id, $point)
{
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if($row['po_expired'] == 100 && ($row['po_expire_date'] == '9999-12-31' || $row['po_expire_date'] >= BV_TIME_YMD))
            $po_expired = 0;
    ...
    if($config['cf_point_term'] > 0) {
        ...
        $sql = " update shop_point
				  ...
				  where mb_id = '$mb_id'
					...
					and po_expire_date < '".BV_TIME_YMD."' ";
        sql_query($sql);
    }
        ...
        if(preg_match("/\.(gif|jpg|jpeg|png)$/i", $filename))
		{
            ...
            if(is_file($path)) {
                ...
                if($size[2] < 1 || $size[2] > 3) {
                    ...
                    @chmod($dir.'/'.$filename, BV_FILE_PERMISSION);
                }
            }
        }
    }
...
function change_order_status_5($od_no, $delivery='', $delivery_no='')
{
	...
	$sql = " update shop_order
				set dan = '5'
				  , invoice_date = '".BV_TIME_YMDHIS."' ";

	if(is_null_time($od['delivery_date'])) // 배송일이 비었나?
		$sql .= " , delivery_date = '".BV_TIME_YMDHIS."' ";
...
function insert_popular($pt_id, $str)
{
	...
	if(!preg_match("/bot|slurp/", $agent)) {
        $sql = " insert into shop_popular set pt_id = '{$pt_id}', pp_word = '{$str}', pp_date = '".BV_TIME_YMD."', pp_ip = '{$_SERVER['REMOTE_ADDR']}' ";
        sql_query($sql, FALSE);
    }
}
...
function display_logo_url($fld='basic_logo')
{
	...
	if(is_file($file) && $row[$fld]) {
		return rpc($file, BV_PATH, BV_URL);
	}
...
function save_goods_data($gs_id, $odrno, $od_id)
{
	...
	{
		$file = BV_DATA_PATH.'/goods/'.$cp['simg1'];
...
function get_skin_stylesheet($skin_path, $dir='')
{
    ...
    $skin_url = BV_URL.str_replace("\\", "/", str_replace(BV_PATH, "", $skin_path));
...
class html_process {
    ...
    function run()
    {
        ...
        if($tmp_row['cnt']) {
            $tmp_sql = " update shop_login set mb_id = '{$member['id']}', lo_datetime = '".BV_TIME_YMDHIS."', lo_location = '{$tb['lo_location']}', lo_url = '{$tb['lo_url']}' where lo_ip = '{$_SERVER['REMOTE_ADDR']}' ";
            sql_query($tmp_sql, FALSE);
        } else {
            $tmp_sql = " insert into shop_login ( lo_ip, mb_id, lo_datetime, lo_location, lo_url ) values ( '{$_SERVER['REMOTE_ADDR']}', '{$member['id']}', '".BV_TIME_YMDHIS."', '{$tb['lo_location']}',  '{$tb['lo_url']}' ) ";
            sql_query($tmp_sql, FALSE);

            // 시간이 지난 접속은 삭제한다
            sql_query(" delete from shop_login where lo_datetime < '".date("Y-m-d H:i:s", BV_SERVER_TIME - (60 * 10))."' ");
        ...
        if(!empty($links)) {
            ...
            foreach($links as $link) {
                ...
                $link[1] = preg_replace('#\.css([\'\"]?>)$#i', '.css?ver='.BV_CSS_VER.'$1', $link[1]);
        ...
        if(!empty($scripts)) {
            ...
            foreach($scripts as $js) {
                ...
                $js[1] = preg_replace('#\.js([\'\"]?>)$#i', '.js?ver='.BV_JS_VER.'$1', $js[1]);
...
function certify_count_check($mb_id, $type)
{
    ...
    $sql .= " and cr_method = '".$type."' and cr_date = '".BV_TIME_YMD."' ";
