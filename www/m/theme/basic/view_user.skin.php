...
<div id="sit_review">
	...
	for($i=0; $row=sql_fetch_array($result); $i++)
	{
		...
		if(is_admin() || ($member['id'] == $row['mb_id'])) {
			echo "<a href=\"javascript:window.open('".BV_MSHOP_URL."/orderreview.php?gs_id=$row[gs_id]&me_id=$row[index_no]&w=u');\" class=\"marl10 tu fc_blk\">수정</a>\n";
			echo "<a href=\"".BV_MSHOP_URL."/orderreview_update.php?gs_id=$row[gs_id]&me_id=$row[index_no]&w=d&hash=$hash&p=1\" class=\"marl5 tu fc_blk itemqa_delete\">삭제</a>\n";
		}
		echo "</span>\n";
		echo "</li>\n";
	}
