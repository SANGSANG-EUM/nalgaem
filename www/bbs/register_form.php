...
if($w == "") {
    ...
    if(!isset($_POST['agree']) || !$_POST['agree']) {
        alert('회원가입 약관 내용에 동의하셔야 회원가입 하실 수 있습니다.', BV_BBS_URL.'/register.php');
    }

    if(!isset($_POST['agree2']) || !$_POST['agree2']) {
        alert('개인정보 수집 및 이용 내용에 동의하셔야 회원가입 하실 수 있습니다.', BV_BBS_URL.'/register.php');
    }
    ...
    if(!$is_member)
        alert('로그인 후 이용하여 주십시오.', BV_URL);

    if($member['id'] == 'admin')
        alert('관리자의 회원정보는 관리자 화면에서 수정해 주십시오.', BV_URL);
